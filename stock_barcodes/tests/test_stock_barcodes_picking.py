...
class TestStockBarcodesPicking(TestCommonStockBarcodes):
    ...
    def setUpClass(cls):
        ...
        # Keep setUpClass focused on creating master data. Wizards are created in setUp()
        # so each test gets a clean one.
        cls.picking_in_01.action_confirm()
        action = cls.picking_in_01.action_barcode_scan()
        assert action.get("type") == "ir.actions.client"
        assert action.get("tag") == "stock_barcode_session"
    ...
    def setUp(self):
        ...
        # Fresh wizards per test: many tests mutate state and a shared wizard breaks
        # expectations when the test order changes.
        self.wiz_scan_picking = self.ScanReadPicking.create(
            {
                "picking_id": self.picking_in_01.id,
                "res_model_id": self.stock_picking_model.id,
                "res_id": self.picking_in_01.id,
                "picking_type_code": self.picking_in_01.picking_type_code,
                "option_group_id": self.barcode_option_group_in.id,
                "manual_entry": self.barcode_option_group_in.manual_entry,
                "picking_mode": "picking",
            }
        )
        self.wiz_scan_picking.location_id = self.picking_in_01.location_id
        self.wiz_scan_picking.location_dest_id = self.picking_in_01.location_dest_id
        self.wiz_scan_picking.onchange_picking_id()
        self.wiz_scan_picking_out = self.ScanReadPicking.create(
            {
                "picking_id": self.picking_out_01.id,
                "res_model_id": self.stock_picking_model.id,
                "res_id": self.picking_out_01.id,
                "picking_type_code": self.picking_out_01.picking_type_code,
                "option_group_id": self.barcode_option_group_out.id,
                "manual_entry": self.barcode_option_group_out.manual_entry,
                "picking_mode": "picking",
            }
        )
        self.wiz_scan_picking_out.location_id = self.picking_out_01.location_id
        self.wiz_scan_picking_out.location_dest_id = (
            self.picking_out_01.location_dest_id
        )
        self.wiz_scan_picking_out.onchange_picking_id()

    def test_picking_wizard_scan_product(self):
        # Work on a fresh picking so previous tests don't affect scanned quantities.
        self.picking_in_01 = self.picking_in_01.copy()
        self.picking_in_01.action_confirm()
        # Recreate the wizard fully bound to the new picking. Swapping picking_id on
        # an existing wizard keeps stale todo/candidate records and breaks scan logic.
        wiz_scan_picking = self.ScanReadPicking.create(
            {
                "picking_id": self.picking_in_01.id,
                "res_model_id": self.stock_picking_model.id,
                "res_id": self.picking_in_01.id,
        ...
        wiz_scan_picking.location_id = self.picking_in_01.location_id
        wiz_scan_picking.location_dest_id = self.picking_in_01.location_dest_id
        wiz_scan_picking.onchange_picking_id()
        wiz_scan_picking = wiz_scan_picking.with_context(force_create_move=True)
        # Scan product and validate (set qty + confirm) to actually write quantities
        self.action_barcode_scanned(wiz_scan_picking, "8480000723208")
        wiz_scan_picking.product_qty = 1.0
    ...
    def test_picking_wizard_scan_product_manual_entry(self):
        ...
        self.picking_in_01 = self.picking_in_01.copy()
        self.picking_in_01.action_confirm()
        # Same as above: recreate wizard to ensure computed todo/actions match the
        # new picking.
        wiz_scan_picking = self.ScanReadPicking.create(
            {
                "picking_id": self.picking_in_01.id,
        ...
        wiz_scan_picking.onchange_picking_id()
        wiz_scan_picking = wiz_scan_picking.with_context(force_create_move=True)
        wiz_scan_picking.action_clean_values()
        self.action_barcode_scanned(wiz_scan_picking, "8480000723208")
        sml = self.picking_in_01.move_line_ids.filtered(
            lambda x: x.product_id == self.product_wo_tracking
        )
        # In manual entry mode, qty is entered by the user before confirming.
    ...
    def test_barcode_from_operation(self):
        ...
        # action_barcode_scan() now opens the OWL client action, so create the
        # wizard explicitly for testing the business flow.
        action = self.picking_type_out.action_barcode_scan()
        self.assertEqual(action.get("type"), "ir.actions.client")
        self.assertEqual(action.get("tag"), "stock_barcode_session")
        # Create a picking-type wizard (no picking selected) so it can find candidates
        # across multiple outgoing pickings.
        self.wiz_scan_picking = self.ScanReadPicking.create(
            {
                "picking_id": False,
                "res_model_id": self.stock_picking_model.id,
                "res_id": False,
                "picking_type_code": self.picking_type_out.code,
                "option_group_id": self.barcode_option_group_out.id,
                "manual_entry": True,
                "picking_mode": "picking",
            }
        )
        self.wiz_scan_picking.location_id = self.stock_location
        self.wiz_scan_picking.location_dest_id = self.customer_location
        self.wiz_scan_picking._set_candidate_pickings(self.picking_out_01)
    ...
    def test_picking_wizard_scan_product_auto_lot(self):
        ...
        # Ensure required screen values are filled so the product barcode matches.
        # (Without view-driven defaults, the wizard may have empty locations.)
        self.wiz_scan_picking.location_id = self.picking_in_01.location_id
        self.wiz_scan_picking.location_dest_id = self.picking_in_01.location_dest_id
        self.wiz_scan_picking.action_clean_values()
        # Scan product with tracking lot enable
